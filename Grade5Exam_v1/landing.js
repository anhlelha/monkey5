...
(function () {
  ...
  if (reduce) {
    ...
    rows.forEach(function (row) {
      ...
      row.querySelector(".meter i").style.transition = "none";
      row.querySelector(".meter i").style.width = target + "%";
      row.querySelector(".pct").textContent = target + "%";
    });
  ...
  /* ---- plan CTAs -> head to the demo app ---- */
  document.querySelectorAll("[data-plan]").forEach(function (a) {
    a.addEventListener("click", function (e) {
      e.preventDefault();
      window.location.href = "Cùng Khỉ con vào lớp 6 CLC.html";
    });
  });
})();

/* =========================================================
   Theme + Tweaks panel (vanilla host protocol)
   ========================================================= */
(function () {
  "use strict";
  var defaults = (window.TWEAKS && typeof window.TWEAKS === "object") ? window.TWEAKS : { theme: "clay" };
  var state = { theme: defaults.theme || "clay" };

  var swatches = Array.prototype.slice.call(document.querySelectorAll("#themeSwatches .theme-swatch"));
