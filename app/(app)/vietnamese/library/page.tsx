...
import { VietnameseLibraryView } from "./VietnameseLibraryView";

export default async function VietnameseLibraryPage() {
  ...
  const [exams, attempts, mastery, profiles] = await Promise.all([
    prisma.exam.findMany({
      where: { subject: "vietnamese", ownerUserId: null, kind: "official" },
    ...
    computeMastery(session.user.id, "vietnamese"),
  ...
  // Only schools that actually have vietnamese exams (keeps the chip row honest).
  ...
  return <VietnameseLibraryView exams={enriched} schools={schools} readiness={readiness} userDone={userDone} />;
}
