...
import { ENGLISH_TOPICS, englishTopicById } from "@/lib/subjects";
...
export default async function EnglishHome() {
  ...
  const [mastery, profiles, activity, enAttempts, activitySeries, streak] = await Promise.all([
    computeMastery(user.id, "english"),
    getAllSchoolProfiles("english"),
    getActivityStats(user.id),
    prisma.attempt.findMany({
      where: { userId: user.id, submitted: true, exam: { subject: "english" } },
      include: { exam: { select: { school: true, year: true, title: true } } },
      orderBy: { createdAt: "desc" },
      take: 5,
    }),
    getUserActivitySeries(user.id, "english"),
    getUserStreak(user.id, "english"),
  ]);
  const readiness = computeAllReadiness(mastery.topicMastery, mastery.levelMastery, profiles);
  const recent = enAttempts;

  // Schools that have english content → readiness cards (mirrors math 4-school grid).
  const SCHOOLS_EN = SCHOOLS.filter((s) => profiles[s.id]);
  ...
  const weakestEntry = practiced.sort((a, b) => a[1] - b[1])[0] ?? [ENGLISH_TOPICS[0].id, BASELINE_MASTERY];
  const weakest = englishTopicById(weakestEntry[0]) ?? ENGLISH_TOPICS[0];

  // Gap advice toward the primary target school (if it has an english profile).
  ...
  if (primaryTargetId && profiles[primaryTargetId]) {
    const profile = await getSchoolProfile(primaryTargetId, "english");
    ...
    if (profile && targetSchool) {
      const items = computeGapTop3(mastery.topicMastery, profile, 0.7, 3).map((g) => ({
        topicId: g.topicId,
        topicName: englishTopicById(g.topicId)?.name ?? g.topicId,
  ...
  const radarData = ENGLISH_TOPICS.map((t) => ({ label: t.short, value: mastery.topicMastery[t.id] ?? BASELINE_MASTERY }));
      ...
      <div className="content">
        <div className="page-head">
          ...
          <div className="row" style={{ gap: 10 }}>
            <Link href="/english/library" className="btn">
              <Icon name="library" /> Xem đề mẫu
            </Link>
            <Link href="/english/topics" className="btn primary">
        ...
        {SCHOOLS_EN.length > 0 && (() => {
          const top = SCHOOLS_EN.map((s) => ({ ...s, r: readiness[s.id] ?? 50 })).sort((a, b) => b.r - a.r)[0];
          return top ? (
        ...
        <div className="grid cols-4">
          {SCHOOLS_EN.map((s) => {
            ...
            return (
              <Link key={s.id} href="/english/library" className={"school-card " + s.tone}>
            ...
            <div className="col" style={{ gap: 12 }}>
              {gapAdvice.items.map((g, i) => (
                <div key={g.topicId} className="row" style={{ gap: 12, alignItems: "center" }}>
                  ...
                  <Link href={`/english/practice?topic=${g.topicId}`} className="btn sm">Luyện</Link>
                </div>
              ))}
            </div>
          </Card>
        ...
        <div className="grid cols-2" style={{ marginTop: 24 }}>
          <Card
            ...
            action={<Link href="/english/topics" className="btn sm ghost">Luyện chuyên đề <Icon name="chevR" size={12} /></Link>}
          >
        ...
        <div className="grid cols-2" style={{ marginTop: 16, gridTemplateColumns: "2fr 1fr" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
              {recent.length === 0 ? (
                <div className="muted" style={{ padding: "24px 0", textAlign: "center", fontSize: 13 }}>
                  ...
                  <Link href="/english/library" style={{ color: "var(--accent)", textDecoration: "underline" }}>Đề thi mẫu</Link>{" "}để bắt đầu.
                </div>
              ) : (
                recent.map((a, i) => {
                  ...
                  return (
                    <Link key={a.id} href={`/exam/${a.examId}/results/${a.id}`} className="row"
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 500, fontSize: 13.5 }}>{a.exam.title ?? `Tiếng Anh ${a.exam.year}`}</div>
          ...
          <div className="col" style={{ gap: 16 }}>
            <Card>
              ...
              <Link href={`/english/practice?topic=${weakest.id}`} className="btn primary" style={{ width: "100%", justifyContent: "center", marginTop: 14 }}>
        ...
        <Card
          ...
          action={<Link href="/english/topics" className="btn sm ghost">Mở luyện chuyên đề <Icon name="chevR" size={12} /></Link>}
          style={{ marginTop: 24 }}
        >
          <table className="tbl">
            <thead>
                ...
                return (
                  <tr key={t.id}>
                    ...
                    <td style={{ textAlign: "right" }}>
                      <Link href={`/english/practice?topic=${t.id}`} className="btn sm">Luyện <Icon name="chevR" size={11} /></Link>
                    </td>
                  </tr>
                );
              })}
