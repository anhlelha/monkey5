...
import { ENGLISH_TOPICS, ENGLISH_SKILLS } from "@/lib/subjects";
import { TopBar } from "@/components/TopBar";
import { Icon } from "@/components/Icon";
import { Bar, Pill } from "@/components/ui";

// English topic practice — mirrors the math /topics grid, grouped by the 5 skills.
export default async function EnglishTopicsPage() {
  const session = await auth();
  if (!session?.user?.id) redirect("/signin");
  const dbUser = await prisma.user.findUnique({ where: { id: session.user.id } });
  if (!dbUser) redirect("/signin");

  const mastery = await computeMastery(session.user.id, "english");

  return (
    <div className="main">
      <TopBar crumbs={[{ label: "Tiếng Anh", href: "/english" }, "Luyện chuyên đề"]} />
      <div className="content">
        <div className="page-head">
          <div>
            <h2>Luyện theo 10 chuyên đề Tiếng Anh</h2>
            <p>5 kỹ năng — Ngữ âm, Kiến thức ngôn ngữ, Giao tiếp, Đọc hiểu, Viết. Phần trăm là mức thành thạo ước tính từ các bài đã làm.</p>
          </div>
        </div>

        {ENGLISH_SKILLS.map((skill) => {
          const ts = ENGLISH_TOPICS.filter((t) => t.skill === skill.id);
          return (
            <div key={skill.id} style={{ marginBottom: 20 }}>
              ...
              <div className="grid cols-4" style={{ gap: 14 }}>
                {ts.map((t) => {
                  ...
                  return (
                    <Link key={t.id} href={`/english/practice?topic=${t.id}`} className="topic-card">
